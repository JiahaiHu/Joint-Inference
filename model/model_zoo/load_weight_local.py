...
def load_checkpoint_from_torch(ckpt_file_name=None,torch_file_name=None,mind_2_torch_dict=None,net=None, strict_load=False,
                             filter_prefix=None, dec_key=None, dec_mode="AES-GCM"):
    if ckpt_file_name==None:
        ckpt_file_name = "./LoadPretrainedModel/resnet50_224.ckpt"
        torch_file_name = "./model_zoo/resnet-50-ckpt.pth"
        with open('./model_zoo/resnet_50_mind_2_torch_key.pkl', 'rb') as handle:
            mind_2_torch_dict = pickle.load(handle)
    ...
    torch_para_dict = torch.load(torch_file_name,map_location="cpu")['model']
    parameter_dict = {}
    try:
        param_data_list = []
        for element_id, element in enumerate(checkpoint_list.value):
            ...
            element_data = torch_para_dict[mind_2_torch_dict[element.tag]].cpu().numpy()#np.frombuffer(data, np_type)
            parameter_dict[element.tag] = Parameter(Tensor(element_data, ms_type), name=element.tag)
    ...
    if net is not None:
        ...
        #print("=====++++++++++++++++++++++")
    # layer_info = {'name':[],"shape":[]}
    # for name in parameter_dict:
    #     layer_info['name'].append(name)
    #     layer_info['shape'].append(parameter_dict[name].data.shape)
    # pd.DataFrame(layer_info).to_excel("./model_zoo/resnet50_mindspore_layer_info.xlsx")
