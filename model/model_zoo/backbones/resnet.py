...
class ResNet(nn.Cell):
    ...
    def __init__(self,
        ...
        self.layer_list = [None, self.conv1, self.max_pool, self.layer1, self.layer2, self.layer3, self.layer4]
        self.layer_name_list = ['input', 'conv1', 'max_pool', 'layer1', 'layer2', 'layer3', 'layer4', 'neck',
                                'prediction']
        self.first_layer_idx = self.layer_name_list.index(first_layer)
        self.end_layer_idx = self.layer_name_list.index(end_layer)
        #print("+++++++###++++++++", self.first_layer_idx, self.end_layer_idx, self.first_layer, self.end_layer)
    ...
    def _make_layer(self,
        ...
        for _ in range(1, block_nums):#block_nums
            layers.append(
                block(
                    self.in_channels,
                    channel,
    ...
    def construct(self, x):
        # print("##################", self.end_layer_idx, self.first_layer_idx)
        if self.end_layer == 'input':
            return x

        for i in range(1, len(self.layer_list)):
            if self.first_layer_idx >= i:
                continue
            else:
                layer_list = self.layer_list[i]
                # print(layer_list[0])
                x = layer_list(x)
                if i >= self.end_layer_idx:
                    break
        # print(x.shape)
        return x
        # x = self.conv1(x)
